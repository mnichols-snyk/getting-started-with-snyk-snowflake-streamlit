...
df_snowpark = session.sql("SELECT o.DISPLAY_NAME,p.NAME,i.GROUP_PUBLIC_ID,i.PROBLEM_TITLE,i.SCORE,i.ISSUE_SEVERITY,i.ISSUE_STATUS,P.TARGET_DISPLAY_NAME,P.ORIGIN,P.PROJECT_OWNER_USERNAME,P.PROJECT_TAGS,p.PROJECT_COLLECTIONS,i.LAST_INTRODUCED,i.LAST_RESOLVED from SNYK.SNYK.ISSUES__V_1_0 i INNER JOIN SNYK.SNYK.PROJECTS__V_1_0 p ON i.PROJECT_PUBLIC_ID = p.PUBLIC_ID INNER JOIN SNYK.SNYK.ORGS__V_1_0 o ON i.ORG_PUBLIC_ID = o.PUBLIC_ID WHERE i.group_public_id='b05f8e68-b63b-4679-b77b-9df4c380d52a' ORDER BY SCORE DESC;")
...
gh_enterprise_repos = df[(df["ORIGIN"] == "github-enterprise")]["TARGET_DISPLAY_NAME"].drop_duplicates().tolist()
